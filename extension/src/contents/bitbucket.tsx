...
import { resolveBitbucketRepo } from "../lib/resolvers/bitbucket"
...
export const getInlineAnchor: PlasmoGetInlineAnchor = () => ({
  element: document.querySelector("[data-aui-sidebar]") ?? document.querySelector(".sidebar") ?? document.querySelector("main"),
...
export default function BitbucketPanel() {
  ...
  const loadScore = useCallback(async () => {
    ...
    const resolved = await resolveBitbucketRepo(`${repo.owner}/${repo.name}`)
    if (key !== prevKey.current) return

    if (!resolved) {
      setState({ score: null, loading: false, error: "Bitbucket project has no linked GitHub repository. RepoRank scores are GitHub-based for now." })
      return
    }
