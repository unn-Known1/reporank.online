...
import { resolveCodebergRepo } from "../lib/resolvers/codeberg"
...
export const getInlineAnchor: PlasmoGetInlineAnchor = () => ({
  element: document.querySelector(".repository-sidebar") ?? document.querySelector(".flex.gap-4") ?? document.querySelector("main"),
...
function QueuedState({ onRetry }: { onRetry: () => void }) {
  const t = { bg: "#ffffff", text: "#24292f", muted: "#57606a", border: "#d0d7de" }
  return (
    <div
      style={{
...
export default function CodebergPanel() {
  ...
  const loadScore = useCallback(async () => {
    if (!repo) { setState({ score: null, loading: false }); return }
    const key = `${repo.owner}/${repo.name}`
    prevKey.current = key

    setState({ score: null, loading: true, queued: false })

    const resolved = await resolveCodebergRepo(`${repo.owner}/${repo.name}`)
    if (key !== prevKey.current) return

    if (!resolved) {
      setState({ score: null, loading: false, error: "Codeberg repository has no linked GitHub repository. RepoRank scores are GitHub-based for now." })
      return
    }

    const result = await fetchScore("github", resolved.github_owner, resolved.github_repo)
    if (key !== prevKey.current) return
    ...
    if (!result.data) {
      await queueLookup(`https://github.com/${resolved.github_owner}/${resolved.github_repo}`)
  ...
  const handleRetry = useCallback(() => { loadScore() }, [loadScore])
  ...
  }, [state.queued, loadScore])

  if (state.loading) return <ScorePanelLoading />
  if (state.queued) return <QueuedState onRetry={handleRetry} />
  if (state.error) return <ErrorDisplay error={{ type: "UNKNOWN", message: state.error }} compact />
