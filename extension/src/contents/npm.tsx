...
import { resolveNpmPackage } from "../lib/resolvers/npm"
...
export const getInlineAnchor: PlasmoGetInlineAnchor = () => ({
  element: document.querySelector("[data-sidebar]") ?? document.querySelector(".sidebar"),
...
export default function NpmPanel() {
  ...
  const loadScore = useCallback(async () => {
    ...
    const resolved = await resolveNpmPackage(repo.name)
    if (key !== prevKey.current) return

    if (!resolved) {
      setState({ score: null, loading: false, error: "No linked GitHub repository found in package metadata. RepoRank scores are GitHub-based for now." })
      return
    }
