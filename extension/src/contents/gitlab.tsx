...
import { resolveGitLabRepo } from "../lib/resolvers/gitlab"
...
export const getInlineAnchor: PlasmoGetInlineAnchor = () => ({
  element: document.querySelector(".project-page-sidebar") ?? document.querySelector(".content-wrapper"),
...
export default function GitLabPanel() {
  ...
  const loadScore = useCallback(async () => {
    ...
    const resolved = await resolveGitLabRepo(`${repo.owner}/${repo.name}`)
    if (key !== prevKey.current) return

    if (!resolved) {
      setState({ score: null, loading: false, error: "GitLab project has no linked GitHub repository. RepoRank scores are GitHub-based for now." })
      return
    }
