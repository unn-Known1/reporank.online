...
export default function EvidenceFacts({ repo }: Props) {
  // Always include stars as the first non-status fact if available
  const priorityFacts: FactItem[] = [];
  const secondaryFacts: FactItem[] = [];
  ...
  if (repo.archived)
    priorityFacts.push({
      ...
      label: "Archived",
      value: "No longer actively maintained",
      variant: "warning",
    });
  ...
  const allFacts = [...priorityFacts, ...secondaryFacts];
  const display = allFacts.slice(0, 3);
